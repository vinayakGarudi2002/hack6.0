...
router.post('/order', isAuth,async (req, res) => {
    ...
    try{
        ...
        if (!user.orderProducts.includes(obj)) {
            await User.updateOne({ _id: req.user._id }, { $push: { orderProducts: obj} })
            await User.updateMany({ _id: req.user._id }, {  wishedProducts: [] })
            await Product.updateMany({_id:obj},{$set:{active:"false"}})

            await Product.updateMany({_id:obj},{$set:{byname:user.name}})// need to work
            // await Product.updateMany({_id:obj},{$set:{byemail:user.email}})// need to work
            await Product.updateMany({_id:obj},{$set:{byphn:user.phoneNumber}})
            await Product.updateMany({_id:obj},{$set:{byadd:user.add}})
            await Product.updateMany({_id:obj},{$set:{bystatus:"false"}})
...
router.get('/wishlist/:id', async (req, res) => {
    try {
        ...
        // console.log(req.user)
         user = await (await User.findById(req.user._id).populate('wishedProducts')).toJSON();
        
        var sum=0;

        // User.findById(req.user._id).populate('wishedProducts').wishedProducts.aggregate([ { $match: {} }, { $group:
        //     { _id : null, sum : { $sum: "$price" } }
        //   }])
        //   .then(res => console.log(res[0].sum));
        ((user.wishedProducts).map(x=>{
            sum=sum+ x.price;
        }))
        // console.log(sum)
        res.status(200).json({ wishlist: user.wishedProducts,sum:sum });

        // console.log({ wishlist: user.wishedProducts,sum:sum })
...
router.get('/orderlist/:id', async (req, res) => {
    try {
        let user = await (await User.findById(req.user._id).populate('orderProducts')).toJSON();
        var sum=0;
        ...
        //   }])
        //   .then(res => console.log(res[0].sum));
        ((user.orderProducts).map(x=>{
            sum=sum+ x.price;
        }))
        // console.log(sum)
        res.status(200).json({ orderProducts: user.orderProducts,sum:sum });

        // console.log({orderProducts: user.orderProducts,sum:sum })
...
router.get('/sum/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ orderlist: user.orderProducts });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
});
